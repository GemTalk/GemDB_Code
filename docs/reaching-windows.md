...
**Re-checked on 2026-09-12 against 4.0.0.Alpha1**, where GemDB's pin now sits.
Nothing below changes in kind — there is still no Windows server, and there is
still a Windows client kit — but three details moved. The kits come from
`dl.gemdb.com/<version>/` rather than the public catalog, so every URL here
names a host that does not carry 4.0. The client is
`GemStone64BitClient4.0.0.Alpha1-x86.Windows_NT.zip` (32.7 MB), carrying
`libgcits-4.0.0.Alpha1-64.dll`. And Intel macOS, which this document treats as
a beneficiary of the same work, no longer has an engine published at all — so
the Docker and remote-server options below are the *only* routes to it, not a
